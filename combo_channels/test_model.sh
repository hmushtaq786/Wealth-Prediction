...
#SBATCH -a 0-4%2                                            # 10 trials = 10 jobs, Max 3 trials at once
#SBATCH -n 1
#SBATCH -c 8    # Each trial gets 16 CPU cores
#SBATCH --output=../../slurm_logs/model_testing/efficientnet/msavi/trial_%A_%a.out      # where to write output, %x give job name, %j names job id
#SBATCH --error=../../slurm_logs/model_testing/efficientnet/msavi/trial_%A_%a.err       # where to write slurm error
...
srun python -u test_model.py --trial-id $SLURM_ARRAY_TASK_ID --indices $INDICES --model $MODEL
