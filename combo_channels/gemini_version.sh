#!/bin/bash
#SBATCH --job-name=train-model-gemini
...
#SBATCH --output=../../slurm_logs/model_training/gemini/training-score_trial_%A_%a.out      # where to write output, %x give job name, %j names job id
#SBATCH --error=../../slurm_logs/model_training/gemini/training-score_trial_%A_%a.err       # where to write slurm error
...
srun python -u gemini_version.py --trial-id $SLURM_ARRAY_TASK_ID --indices $INDICES --model $MODEL
