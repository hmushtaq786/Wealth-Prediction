#!/bin/bash
#SBATCH --job-name=train-efficientnet-gpu-optuna
...
#SBATCH --output=../slurm_files/efficientnet/slurm-%x-%j.out     # where to write output, %x give job name, %j names job id
#SBATCH --error=../slurm_files/efficientnet/slurm-%x-%j.err      # where to write slurm error
...
python -u cnn_efficientnet_optuna.py
