#!/bin/bash
#SBATCH --job-name=train-resnet-gpu
...
#SBATCH --output=../slurm_files/new//slurm-%x-%j.out     # where to write output, %x give job name, %j names job id
#SBATCH --error=../slurm_files/new/slurm-%x-%j.err      # where to write slurm error
