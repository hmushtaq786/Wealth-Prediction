...
class SatelliteWealthDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.dataframe.iloc[idx]['wealth']
...
def match_images_to_labels(csv_path, image_folder, target_column):
    ...
    for file in image_files:
        ...
        matches.append({'image_path': file, 'wealth': wealth_value})
...
def objective(trial):
    ...
    torch.save(model.state_dict(), f"optuna/models/resnet34-test/trial_{trial.number}.pth")

    # Save R² scores per epoch for this trial
    np.save(f"optuna/scores/resnet34-test/trial_{trial.number}_r2.npy", np.array(test_r2s))
...
def plot_optuna_study(study, scores_path):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("optuna/plots/resnet34-test.png")
    plt.show()
...
def main():
    ...
    scores_path = "optuna/scores/resnet34-test/trial"
    plot_optuna_study(study, scores_path)

    # Save the Optuna study
    joblib.dump(study, "optuna/studies/resnet34-test.pkl")
