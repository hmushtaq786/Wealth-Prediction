#!/bin/bash
#SBATCH --job-name=train-resnet-wealth-test-gpu-optuna
...
#SBATCH --output=../slurm_files/resnet-test/slurm-%x-%j.out     # where to write output, %x give job name, %j names job id
#SBATCH --error=../slurm_files/resnet-test/slurm-%x-%j.err      # where to write slurm error
...
python -u 8_cnn_resnet_wealth_test_optuna.py
