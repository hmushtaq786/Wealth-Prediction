...
def objective(trial):
    ...
    torch.save(model.state_dict(), f"optuna/models/efficientnetb3/trial_{trial.number}.pth")

    # Save R² scores per epoch for this trial
    np.save(f"optuna/scores/efficientnetb3/trial_{trial.number}_r2.npy", np.array(test_r2s))
...
def plot_optuna_study(study, scores_path):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("optuna/plots/efficientnetb3.png")
    plt.show()
...
def main():
    ...
    scores_path = "optuna/scores/efficientnetb3/trial"
    plot_optuna_study(study, scores_path)

    # Save the Optuna study
    joblib.dump(study, "optuna/studies/efficientnetb3.pkl")
