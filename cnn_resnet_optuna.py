...
def objective(trial):
    ...
    torch.save(model.state_dict(), f"optuna/models/resnet34/trial_{TRIAL_ID}.pth")

    # Save R² scores per epoch for this trial
    print(trial)
    np.save(f"optuna/scores/resnet34/trial_{TRIAL_ID}_r2.npy", np.array(test_r2s))
...
def plot_optuna_study(study, scores_path):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("optuna/plots/resnet34.png")
    plt.close()
